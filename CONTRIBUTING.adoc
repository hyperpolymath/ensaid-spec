...
. *Write spec language* using RFC 2119 keywords (MUST, SHOULD, MAY).
. *Include conformance criteria* – how would an implementation test
compliance?
...
[arabic]
. Open an issue with the proposed section’s abstract and motivation.
. Discuss with maintainers before writing the full section.
. Follow the existing section structure (Abstract, Motivation,
